...
func (packet *packet) write(b *bytes.Buffer) {
	...
	if packet.sequencedOrOrdered() {
		...
		b.WriteByte(packet.orderChannel)
	}
	if packet.split {
...
func (p *packet) read(b *bytes.Buffer) error {
	*p = packet{}
	header, err := b.ReadByte()
	if err != nil {
		return fmt.Errorf("error reading packet header: %v", err)
	}
	p.split = (header & splitFlag) != 0
	p.reliability = (header & 224) >> 5
	...
	if p.reliable() {
		p.messageIndex, err = readUint24(b)
		if err != nil {
	...
	if p.split {
		if err := binary.Read(b, binary.BigEndian, &p.splitCount); err != nil {
			return fmt.Errorf("error reading packet split count: %v", err)
		}
		if err := binary.Read(b, binary.BigEndian, &p.splitID); err != nil {
			return fmt.Errorf("error reading packet split ID: %v", err)
		}
		if err := binary.Read(b, binary.BigEndian, &p.splitIndex); err != nil {
			return fmt.Errorf("error reading packet split index: %v", err)
		}
	}

	p.content = make([]byte, packetLength)
	if n, err := b.Read(p.content); err != nil || n != int(packetLength) {
		return fmt.Errorf("not enough data in packet: %v bytes read but need %v", n, packetLength)
	}
	return nil
}

func (packet *packet) frame() *Frame {
	return &Frame{
		Payload:      packet.content,
		Reliability:  Reliability(packet.reliability),
		OrderChannel: packet.orderChannel,
	}
}

func (packet *packet) reliable() bool {
	return packetReliable(packet.reliability)
}

func (packet *packet) sequencedOrOrdered() bool {
	return packetNeedsOrderIndex(packet.reliability)
}

func (packet *packet) sequenced() bool {
	return packetNeedsSequenceIndex(packet.reliability)
}

func packetReliable(reliability byte) bool {
	switch reliability {
...
func packetNeedsOrderIndex(reliability byte) bool {
	switch reliability {
...
func packetNeedsSequenceIndex(reliability byte) bool {
	switch reliability {
	case reliabilityUnreliableSequenced,
		reliabilityReliableSequenced:
		return true
